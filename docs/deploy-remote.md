...
Step-by-step setup — deploy, connect the Mac, pair a phone: [remote access guide](remote-access.md) · 分步接入：[远程访问指南](remote-access.zh.md)。This page is the operator and contract reference.

**Default-off infrastructure, not a released remote application.** One owner, one execution Mac, at most 16 enrolled devices. No cloud account, official service, arbitrary HTTP proxy, relay decryption, offline command/approval queue, or inbound Mac listener. The daemon adapter (ticket 07) is native-gated. The hosted messenger PWA (ticket 08) pairs through the unregistered mailbox, speaks Noise after enrollment, and is built with `REAL_BOT_HOSTED=1` so `__local-api` and the loopback bearer are not compiled in. Optional Web Push is Mac-outbound only: HTTPS to `*.push.apple.com`, `fcm.googleapis.com` or `updates.push.services.mozilla.com`, `redirect: error`, payload `{t:"pending"}`. The production daemon advertises `push_settings_v2` with `push_transport=policy_v2`. Outbound send, subscribe and test require an open remote gate (activation / native / trust); unsubscribe stays available for cleanup. There is no temporary upgrade pause. Self-hosted operators configure the operator contact URI via local loopback `GET/PATCH /v1/notifications/push-config` (`mailto:` or `https:`, max 512 bytes, no control characters); missing contact configuration pauses outbound push with `push_contact_required` while inbox and local notifications remain fully operational. The relay never sends. Adding a push domain requires code and docs review. Public pairing stays off. Desktop Chromium smoke is not iOS home-screen / G-uv / G-push. [Protocol/security gates](remote-protocol.md) still apply, especially independent security review and physical-device validation. Do not expose public pairing before those gates pass.

**默认关闭的基础设施，不是已发布远控。** 单人、单执行 Mac、最多 16 设备；不引入官方云、账号、任意 HTTP 代理、明文业务、离线命令/批准队列或 Mac 入站监听。daemon 适配器（票 07）仍原生门控；托管信使 PWA（票 08）经未登记邮箱配对、登记后走 Noise，生产包用 `REAL_BOT_HOSTED=1` 编译，不含 `__local-api` 与回环 bearer。可选 Web Push 仅 Mac 出站：HTTPS 到 `*.push.apple.com`、`fcm.googleapis.com` 或 `updates.push.services.mozilla.com`，`redirect: error`，载荷 `{t:"pending"}`。生产 daemon 会广告 `push_settings_v2`，`push_transport` 为 `policy_v2`。出站发送、订阅与测试受远控激活 / 原生 / 信任门约束；解绑仍可用于清理。不再停在临时升级暂停。自托管部署方通过本机回环 `GET/PATCH /v1/notifications/push-config` 配置联系方式（`mailto:` 或 `https:`，不超过 512 字节，无控制字符），未配置时暂停推送并提示 `push_contact_required`，会话列表上的待处理状态与桌面本地通知照常工作；中继不代发。增加推送域名必须代码+文档评审。公网配对仍关闭。桌面 Chromium 冒烟不是 iOS 主屏幕 / G-uv / G-push。独立安全复核、真机、域名/TLS 等发布门仍未通过。
...
pnpm --filter @real-bot/messenger build:hosted
pnpm --filter @real-bot/relay smoke:serve
```

The last command serves only this checkout's hosted production static files at `http://[::1]:5186`, with script-src `'self'` (plus hashes only if inline scripts remain), no Vite, no API and no discovery response. It fails rather than evicting another listener. Use the isolated `agent-browser --session rc08` session to load desktop/narrow pairing, chat and disconnect states. No personal database, Keychain, model service or port 17890 is used. Tests start actual Bun HTTP/WS servers on random loopback ports, generated identities and temporary SQLite; two simultaneous host links carry real `DeviceSession`/`HostSession` Noise traffic. A raw paused TCP/WebSocket reader exercises actual write-buffer backpressure.
...
The execution Mac sends Web Push to the browser-provided subscription service. The daemon honors `https_proxy` / `HTTPS_PROXY`, falling back to `all_proxy` / `ALL_PROXY`, and `no_proxy` / `NO_PROXY` exclusions. Lowercase variables take precedence. Configure an HTTP or HTTPS proxy URL in the environment inherited by the daemon, for example `HTTPS_PROXY=http://127.0.0.1:7890 pnpm dev`. The proxy port must match your own installation. macOS System Settings proxy preferences alone do not populate this environment; a running daemon must be relaunched to inherit environment changes.

The proxy is operator-trusted. Subscription destinations remain restricted to the push-service allowlist; the daemon validates all DNS answers and pins the CONNECT destination to a public IP while preserving the original TLS server name and certificate verification. Redirects remain forbidden. The relay does not gain a push-forwarding endpoint. The phone also needs connectivity to its browser's push service.

Remote test feedback distinguishes queueing, push-service acceptance, and a first-attempt timeout followed by a scheduled retry. Acceptance confirms the service accepted the request; phone presentation still depends on browser and system delivery. The test response may include a fixed `error_code` for a queued retry; subsequent delivery outcomes remain in the daemon's delivery records.
