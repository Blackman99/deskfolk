...
- **Your terminals** — watch and type into a shell the daemon holds, with a two-row key bar for Esc, Tab, Ctrl, arrows and Paste.
- **A Files conversation** of its own — a file sent there is copied into the workspace `inbox/`, text stays as a note to yourself, and neither wakes a Bot.
