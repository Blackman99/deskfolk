...
- 不在地址里的层由 `topLayer(state)` 定顺序（与 `Shell.svelte` 里 Escape 链一致），`Shell.backMobileLayer()` 按它收起一层并返回 true，`+page.svelte` 的 `beforeNavigate` 据此 `cancel()`。地址里带的层（设置、会话抽屉、Bot 资料、工作区、预览）返回 false，交给历史；只有 `blocksClose()` 为真（未保存/正在保存）时才拦下返回让面板先问。放大的图片（`chat/MessageImageLightbox.svelte`，会话、预览和工作台窗格里都会打开）不在地址里、盖住一切，是 `topLayer` 的第一层 `image`：组件挂载时用 `chat/enlarged-images.ts` 的 `holdBack` 登记自己的关闭函数，返回键先把最新那张缩回原图，页面不动；Escape 仍由它自己在捕获阶段处理。两个易错点：`selfBack` 标记本页自己发起的 `history.back()`，不能被当成用户按返回；`cancelledBack` 标记「返回被应用接住」，此后那次变浅要 replace 而不是 back，因为 cancel 会让 SvelteKit 往前补一步，两者相撞就什么都不动（前向的 popstate 不清这个标记）。
