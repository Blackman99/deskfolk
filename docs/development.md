...
- 纯函数模块留在顶层：`route-decision.ts`（候选、评分、经验的正负与封顶）、`models.ts`、`mentions.ts`（引用回复补 `@`；解析本身在 `@real-bot/protocol`）、`context.ts`、`schedule.ts` 等；`turn-engine.ts` 是轮次回路（一轮连续调工具，每满 40 跳在循环里问一次是否卡住，到 160 跳收起工具、下一条回复收尾，见 `turn-pace.ts`；发给 MCP 的参数里整条是 `workspace://<路径>` 的字符串，由 `mcp-workspace-refs.ts` 在发出前换成那张图的 data URI，超过半兆按 `vision-image.ts` 缩小，模型那边的调用保持原样；`read_file` 读到的栅格图同样缩好，由 `loop-pictures.ts` 附在这一跳全部工具结果之后的一条 user 消息里，工具结果本身只写路径和有没有附上，这些图先占 `context.ts` 转录窗口的看图预算），`local-api.ts` 是本机接口。`artifact-paths.ts` 做正文 linkify，路径像不像工作区文件由协议包判定。单独成行的 `附件：<路径>` 和转录里展示附件的那一行是同一种写法，算作交出文件：新消息会挂成附件；已经落库的旧消息由信使收成气泡下的附件卡片，那一行不再留在正文里。卡片或正文里点开的是图片（含 SVG）时，放大层铺满整个应用（`MessageImageLightbox`，`position: fixed`，盖住侧栏和底栏），从点中那张图的视口矩形过渡到居中的大图，收起时缩回原处；Esc、关闭或点空白收起，不打开产物预览、也不写进 URL。字节还在路上时，放大框先从那张小图的矩形长到居中的舞台（和最终大图同一块区域），框里画转圈；附件已知大小或传输回报了总量时进度条从 0 填到百分比并写出已传字节，否则进度条来回扫。文案和进度条等框长过缩略图才画出来，转圈从点击起就在，框底是实色，免得转圈和字节数压在转录上读不清。图片字节到达后再按原图比例收成最终尺寸。缩略图自己也在原位置转圈（附件芯片 36px、正文图片链接 72×54），不先换成文件图标。正文里写到的工作区图片路径（Bot 的回复和你的消息；还在流式输出的回复不画，每来一段都会重建链接）由 `ChatStage` 给 `MarkdownBody` 传 `loadArtifactImage`，画成缩略图芯片：取 Mac 的 256px 副本（`size: 'thumb'`、`background: true`，和附件芯片一样排在主动打开的请求后面），滚到附近才取，正文卸载时用传进去的 `signal` 取消。链接文字就是路径本身时芯片只写文件名、完整路径放进 `title`；取不到就恢复成原来的链接和文字。取字节走已有的 `onProgress`。系统开了「减少动态效果」时放大直接到位，转圈和扫动停住。混有非图片的一批、单个非图片、右键「打开相关文件树」和工作区里点开的文件仍走预览。附件卡片（含多文件文件夹）在用户与 Bot 气泡中共用 `--pane` 底色和 `--ink` 文字；行内附件缩略图使用 `--line-subtle`，图片放大层的遮罩使用混入透明度的 `--bg`，图片和加载框使用实色 `--pane`；按钮、文件名、失败提示和进度使用同一套主题令牌，亮暗主题切换时即时更新。`chat/image-theme.test.ts` 覆盖已打开的图片、加载、失败和缩略图底色，并在文件会话、私聊和群聊的真实消息气泡内检查单图与文件夹卡片的主题配色。这些图（缩略图、正文里的图、放大层、文件预览、流程图里的图，以及输入框里待发送的图）右键是「复制图片」：`Shell` 上的 `ImageCopy` 在捕获阶段接住带 `data-copy-image` 的 `img`，菜单调 `copyRenderedImage`，在同一次点击里把 `ClipboardItem` 交给剪贴板，像素以 PNG 写入。头像和图标没有这个标记。按住 ⌥ 右键不拦截，留给网页视图自己的菜单。消息菜单和窗格分割菜单都让开这张图。远程连接下放大层 ✕ 左边多一个「下载原图」：存的是原图，不是屏幕上的 1600px 副本，和「查看原图」共用同一次下载（`fetchOriginal`，取回后同样原地替换副本）。`save-file.ts` 在触屏（`pointer: coarse`）上把文件交给系统分享面板：iOS 上只有分享面板的「存储图像」能进相册，`<a download>` 指向 blob 时会静默无效。其余浏览器照常下载。分享面板只在点按当下能打开：原图已在手里时同一次点按就开（中间不能有 await）；等下载等过了头被拒（`NotAllowedError`）时不改成下载，按钮变强调色，图下写「原图已取回，再点一次下载」。本机窗口不显示这个按钮。

## 手机宽度导航

680px 及以下由 `Shell.svelte` 组合 `MobileNavigation.svelte`，共享会话、工作区和设置三个主入口。设置根页显示底栏，分类详情和编辑页隐藏底栏；会话详情沿用顶部返回。`Sidebar.svelte` 的归档入口在搜索旁的工具菜单里（与日程图、花费、终端并列；手机仍叫终端，打开终端页），数量只在菜单内灰色显示，入口按钮上不再带点。顶栏由左侧功能入口按钮与通栏胶囊搜索条组成，搜索打开全局弹窗的手机全屏布局（见下文「手机返回」），进入已归档视图后顶部提供专属导航页头与返回按钮；新建则是列表右下角的浮动 `+`（`.fab`，点开选「新建 Bot / 新建群」，分组标题里那两个 `+` 在手机上隐藏）。宽于 680px 的侧栏底部是带文字的「工作区 / 工具 / 设置」，不再是七个图标；工具菜单依次是日程、花费、新建终端，分隔线后是已归档会话。标题里的 `+` 仍在。手机底部导航保留。手机上这一列的左右边界统一：搜索入口、分组标题、头像都从 16px 起；列表在手机宽度下隐藏滚动条。工作区通过 `requestCloseFromParent(afterClose)` 完成未保存确认后执行导航，取消时释放目标回调并保留草稿。配置仍使用已有设置和工作区 URL 状态。
...
打开的是哪个会话记在 URL 的 `?s=<id>` 上，打开的产物预览记在 `?p=<relpath>` 上，设置 / 会话抽屉 / 工作区 / 轨迹 / 日程图 / 手机终端页浮层共用 `?o=`（`session-url.ts`，日程图是 `?o=routines`，手机终端页是 `?o=terminal`），刷新、热更新和后退键都回到同一处。桌面工作台的终端标签仍是一条 shell，不进地址。用查询参数而不是路径，是因为打包后的 Tauri 窗通过资源协议直接服务 `build/`，没有 SPA 回退：`/s/<id>` 一刷新就是 404，而 `index.html?s=<id>&p=<relpath>&o=settings` 永远是磁盘上那个文件。`+page.svelte` 里会话、预览和浮层各有 URL→运行时的 effect，一条写回 URL；各自只跟踪自己那一侧（都跟踪就会互相覆盖）。URL 里的会话 id 在会话列表到达前不动它，`connect()` 拿到列表后会把不存在的 id 清掉。预览路径和工作区开关挂在 `MessengerRuntime` 上、不跟 `Shell` 走：断线那一屏卸掉 `Shell` 再连上时还在。非法预览路径（空、绝对路径、`..` 逃出工作区）当没打开。抽屉要等快照里真有那个会话 / Bot 才打开，避免闪一下错误的面板。模型选择记录、新建弹窗和确认框不进 URL。
...
Bot 资料中的 `RoutineCard.svelte` 读取 `snapshot.routines`，只提供现有每天/每周与 `HH:MM` 字段，归属固定为当前 Bot。时间按执行 Mac 的本地日历解释，不提供浏览器时区转换或新 cron 语法；使用步骤见 [日程说明](routines.zh.md)。680px 及以下，编辑是盖在资料上的整页（`.routine-page`，`pageSlide`），列表只留钟点、标题和启停开关；返回由 `RoutineCard.backFromEditor` 经 `ProfilePane.backFromEditor` 交给 Shell 的 Back / Escape，先于关闭资料。保存中这一页不关闭。宽屏不套卡片：条数在 `ProfilePane` 的标签上，表单展开在所编辑的那一行下面（`.routine-editor-slot`）；新建的、或编辑中被别处删掉的日程没有行可挂，表单落在列表末尾。
...
一件事的中间产物集中在一个目录里，而不是散落在工作区根。一件事就是一个**规划**（`tasks` 表，历史名；`store/tasks.ts`），它下面有**任务**（`tickets` 表，`store/tickets.ts`）；规划的要点和每一版留痕在 `store/plan-shape.ts` / `store/plan-spec.ts`。谁属于哪个规划由**整理跳**决定（`prompts/organizer.ts` 出提示词和载荷，`organizer.ts` 管时机、串行、失败即不改、镜像文件；调用走 `judge`，但自带 `maxTokens: 4096`——短调用默认的 256 / 512 装不下一份要点，答案会断在 JSON 中间——和整段超时，消息 60 秒、settle 120 秒（不流式，超时连答案一起算）；答案截断（`finish_reason: "length"`，`JudgeResult.truncated`）、读不成要点、调用失败或抛错都不改，各写一行 `[organizer]` 日志说明是哪一种；载荷里「上一版之后」的消息和文件取最新的 30 条、按时间正序排；引擎在 `handleInboundMessage` 里先 `await organizer.organizeMessage(message)` 再参与，终态轮经 `noteTurnEnded` 触发 30 秒去抖的 settle）：用户消息被盖上 `messages.task_id` / `ticket_id`，`resolveTurnTask` 按「显式 taskId（回看、批注）→ 唤醒轮继承 → 触发消息上的盖章 → 会话当前规划 → 新开（spec 空，等整理跳补）」定一轮的规划和任务，交接、点名、判断下场、Bot↔Bot 私聊和中断续跑都继承唤醒方那一轮（`turns.task_id` / `turns.ticket_id`）。没有时钟：`sessionCurrentTask` 就是 `closed_at IS NULL AND routine_id IS NULL` 的那一个，直到整理跳判 new / resume 或要点变成 done / parked。日程一条一个常驻规划（`tasks.routine_id`），每次到点 `createTicket` 一个 `NN-<日期>` 再开轮，不调模型。目录名是 `work/<目标或触发消息前 12 码点>-<id 后四位>/`，任务目录是 `<规划目录>/<NN>-<标题 slug>/`；`turnWorkDir` 先给任务目录，`turnPlanDir` 给规划目录；在第一次真被用到时才创建，只说话的轮次不留空壳。整理跳每写一版要点就用 `atomicWrite` 重写 `<规划目录>/map.md` 和每个 `<任务目录>/ticket.md`，两者是保留名，不进产物。
...
交付物不进工作目录：用户点名了路径就照他说的写，没点名的落本轮任务目录（没有任务就是规划目录）。局面块每轮带 `规划：… / 验收 / 规则 / 流程与分工 / 进展 / 任务清单 / 本轮任务 / 先例 / 已交出 / 经过` 和一行 `本轮任务目录：<路径>/（规划目录：<路径>/）`（`context.ts` 的 `planFacts` / `planLines`）；还没整理出要点时退回「这件事最初的要求」那一行。
...
同一件事也能按状态流转看。`GET /v1/tasks/:id` 整个给出一个规划（摘要 + `brief` / `spec` / `revision` / `tickets`，每个任务带产物），`GET /v1/tasks/:id/tickets`、`GET /v1/tasks/:id/spec-revisions` 分别给任务和要点历史，`PATCH /v1/tasks/:id/spec`（整份要点 + `if_revision`，冲突 409）和 `PATCH /v1/tickets/:id` 是用户的改动，记成 actor 为 user 的一版并重写镜像；库的变更日志对 `tasks` / `tickets` 表发 `task.upsert` / `task.removed` / `ticket.upsert` / `ticket.removed`，信使开着这个规划的板子时收到就再拉一次（150 ms 合并）。`GET /v1/tasks/:id/trace` 把共享这个规划的轮次读成一张经过：你的发言一张卡，每个 Bot 的一轮一张卡（带 `ticket_id`），边是触发消息的 `turn_id`（没有就是你），产物是这一轮消息上的附件（每个带 `exists`，从卡片打开预览时删掉的文件不会回到树里），旁观人数记在叫醒它们的那张卡下。信使按这条边从上到下排成流程图（同一轮叫醒的人并排在一行），每张卡片在名字左边放这个人的头像。点卡片跳到这一轮自己的那句话：还在写或已经说完的，是它最后一句；被中断的，是那条「中断」，不是它上面那一句。点一个文件走宿主的预览（板子自己不画文件）。板子标题是规划的目标（`plan-board.ts` 的 `planTitle`），下面是可折叠的要点面板（`PlanSpecPanel.svelte`：每栏原地改、整份要点带 `if_revision` 发回、看每一版的改动记录），流程图旁是任务栏（`TicketList.svelte`：序号、状态、谁在做、产物、跳到最近一轮、改状态）；点一个任务，它的卡片亮、其余暗，和模型高亮互斥；卡片上带任务序号。`@container` 窄于 560px 时任务栏和流程图分段轮流显示，手机页（`host="page"`）要点默认收起。`GET /v1/sessions/:id/tasks` 列出这个会话参与过的事，按最近活动倒序。会话顶栏的「经过」在桌面端开成一块窗格（`TraceView.svelte`；想让它浮起来就按住 ⌥ 把标签拖出去，那是窗格布局的事，板子自己不再带浮动窗；点一个节点，对话切到那一轮所在的会话并高亮，窗格留着，当前会话里的节点加一圈标记；换一条对话，窗格改成那条对话最近的一件事）。手机上是单独的一页（`TaskTrace.svelte` 是那个壳），铺满屏幕，消息菜单的「看这件事」和 Bot↔Bot 私聊来源头打开同一张。从一条消息打开时，板子打开后直接移到这条消息自己的那张卡片，并把它放在视口中间、按原尺寸画；对不上卡片就仍是整张适应。板子已经开着时，再从一条消息打开，画面从当前位置平滑滑到那张卡片，拖动或缩放会停掉这次滑动。顶栏的「经过」不指定卡片。地址是 `?o=trace&k=<taskId>`，开着时该工作的轮次或消息一变就再拉一次。超过 40 张卡片时不再画线，改由卡片自己写「由谁的上一轮叫醒」。手机宽度改成从上到下，预览改到流程图下面。
...
规划目录和任务目录下都有两个保留子目录：`tool-results/`（守护进程的大工具结果）和 `scratch/`（轮次指令让 Bot 放纯过程文件的地方），外加应用的镜像文件 `map.md` / `ticket.md`。**写进这些地方的文件不会自动挂成消息产物**，其余位置照旧；这条规则在 `store/tasks.ts` 的 `isReservedTaskPath`（两层都认），由 `turn-engine.ts` 的 `noteWrittenPaths` 执行。
...
预览为只读，办公文件暂不提供批注或保存。三类预览统一限制文件 50 MiB、ZIP 4000 项、单项解压 32 MiB、总解压 128 MiB；加密、损坏、格式不匹配和容量超限有明确错误提示。切文件或关闭时取消读取并丢弃迟到解析结果，幻灯片渲染器销毁时释放其资源。
...
手机刚传上去的文件不再从 Mac 读回来。`RemoteApi.postMessage` 发完后把 Mac 回的附件行交给 `remote/sent-files.ts` 的 `SentFiles`，按大小（Mac 的 stat）再按文件名对上刚传的 `File`，以附件 id 和 `workspace_relpath` 记下；之后 `getAttachmentBlob`、`getWorkspaceFileBlob`、`openMediaSource` 先问它。只记图片、PDF、音视频：这几样大，Bot 也几乎不会原地改写。文本、Markdown、代码和 SVG 小，Bot 会原地改，仍从 Mac 读。本地那份带上 Mac 会给的 ETag（带引号的 SHA-256，上传时本来就算过），批注的哈希和保存时的 `If-Match` 都和从 Mac 读到的一样；Mac 上的文件后来被改过，保存照样冲突，不会悄悄覆盖。`thumb` 由 `scalePicture` 在手机上画成 256px 副本（照片出 JPEG，可能透明的出 PNG，本来就不大于 256px 的原样返回），并像 Mac 的副本一样记下原图大小；`preview` 直接给原图。画不出来的图（比如这台浏览器不认的 HEIC）缩略图和放大改回 Mac 的副本，原图仍从手机上拿。只放在内存里：远程页面不落盘任何文件，刷新或 PWA 被系统杀掉后就回到从 Mac 读。没对上的一律从 Mac 读，Mac 的读请求仍在调用的同一轮进队，不因为先问了本地而排到后面去。
...
- 消息首次写入发 `message.created`；回应、附件、回答提问或中断 Continue 等后续更新发 `message.upsert`（完整消息含 reactions）。级联删除审批发 `approval.removed`；花费账本保留，`spend.removed` 不再发出。旧本机事件不变。`turn.token` 不进环，若发布则转成 Store 中绝对 `partial_text` 的轮次更新；`turn.tool` 仅保留旧本机流，不进入新同步流。当前引擎仍不把工具中间跳当作用户回复。
...
浏览器不报的那种死链（换网后 socket 仍称 open 却什么都不送）按静默判定：有未回答的请求、发送缓冲与上次检查相比没变、且距最后一帧或该请求发出满 30 秒，才判死。空闲链路不判（没有等待就没有证据），仍在上传的链路不判（缓冲在动就是还在走）。窗口取 30 秒是因为主机侧模型探测每次 12 秒、只有超时才再试一次（最坏约 24 秒），一轮工作期间事件本来就在流；判错的代价是一次握手加一次回执查询，判不出的代价是页面一直装作连着。
...
- 不在地址里的层由 `topLayer(state)` 定顺序（与 `Shell.svelte` 里 Escape 链一致），`Shell.backMobileLayer()` 按它收起一层并返回 true，`+page.svelte` 的 `beforeNavigate` 据此 `cancel()`。地址里带的层（设置、会话抽屉、Bot 资料、工作区、预览、手机终端页 `?o=terminal`）返回 false，交给历史；只有 `blocksClose()` 为真（未保存/正在保存）时才拦下返回让面板先问。手机终端页的返回按钮和 Esc 关掉这一页，shell 仍由守护进程持有。放大的图片（`chat/MessageImageLightbox.svelte`，会话、预览和工作台窗格里都会打开）不在地址里、盖住一切，是 `topLayer` 的第一层 `image`：组件挂载时用 `chat/enlarged-images.ts` 的 `holdBack` 登记自己的关闭函数，返回键先把最新那张缩回原图，页面不动；Escape 仍由它自己在捕获阶段处理。两个易错点：`selfBack` 标记本页自己发起的 `history.back()`，不能被当成用户按返回；`cancelledBack` 标记「返回被应用接住」，此后那次变浅要 replace 而不是 back，因为 cancel 会让 SvelteKit 往前补一步，两者相撞就什么都不动（前向的 popstate 不清这个标记）。
...
- 全局搜索由 `Shell.svelte` 的 `searchOpen` 管理，`search/GlobalSearch.svelte` 使用原生 `dialog.showModal()` 隔离背景和键盘焦点。展开侧栏、头像窄栏与 ⌘K / Ctrl+K 共用入口；`search/shortcuts.ts` 让终端和 Monaco 保留普通 K 绑定，⌘⇧K / Ctrl+Shift+K 在其中仍可打开。桌面宽 640px、结果独立滚动；手机（≤680px）铺满 `visualViewport`，留安全区并隐藏底栏。改变窗口宽度保留输入、分类和结果；取消恢复入口焦点，选择结果交给目标界面。`topLayer` 的 `search` 层先于页面收起，`Shell.backMobileLayer()` 调 `closeGlobalSearch()` 清空查询。跳转先通过 `searchJump` 计算，文件经 `openArtifactPath`、日程经 `openRoutine`、消息经 `selectSession(..., {messageId})`；手机离开工作区或预览时使用现有未保存确认。`runSearch` 每次调用（含空查询）先递增请求序号、清掉旧结果，并维护 `searchLoading` / `searchError`；同一请求且连接未变才接纳结果，关闭、断线和销毁均使在途结果失效。
