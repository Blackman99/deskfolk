...
- Ticket 04 owns this shared ordering. Ticket 02 and daemon/client adapters must consume the exact export, **not** method/path/encoding/body order or a separate If-Match suffix. The mandatory final JCS field changes the previous unpublished digest even when no condition is present. There is no compatibility fallback to the old five-field digest.
