...
export const sha256Hex = (value: string | Uint8Array): string => hex(sha256(typeof value === 'string' ? new TextEncoder().encode(value) : value));
export const canonicalBytes = (value: unknown): Uint8Array => utf8(canonicalize(value));
export const canonicalHash = (value: unknown): string => sha256Hex(canonicalBytes(value));
...
export function normalizeAttachmentDigests<T extends AttachmentDigest>(input: readonly T[]): T[] {
  const files = Array.from({ length: input.length }, (_, index) => {
    check(Object.hasOwn(input, index), 'sparse attachment list');
    ...
    return { file, filenameBytes: utf8(file.filename) };
  });
  files.sort((a, b) => {
    const x = a.filenameBytes, y = b.filenameBytes;
    for (let i = 0; i < Math.min(x.length, y.length); i++) if (x[i] !== y[i]) return x[i] - y[i];
    return x.length - y.length || (a.file.sha256 < b.file.sha256 ? -1 : a.file.sha256 > b.file.sha256 ? 1 : 0);
  });
  return files.map(({ file }) => file);
}

function encodeField(value: unknown, encode: CanonicalEncoder): string {
  const result: unknown = encode(value);
  if (typeof result !== 'string') {
    if (result !== null && (typeof result === 'object' || typeof result === 'function')) {
      // Observe native Promise rejection without awaiting or executing arbitrary thenables.
      try { Promise.prototype.then.call(result, () => {}, () => {}); } catch {}
    }
    throw new Error('canonical encoder must return a synchronous primitive string');
  }
  return result;
}

export function requestDigestBytes(input: RequestDigestInput, encode: CanonicalEncoder = canonicalize): Uint8Array {
  ...
  check(input.encoding !== 'json' || !input.files?.length, 'JSON cannot contain file digests');
  const files = normalizeAttachmentDigests(input.files ?? []);
  // Encoding separates JSON from empty multipart requests, which otherwise collide.
  return utf8([input.method, input.path, encodeField(input.body, encode), input.encoding,
    files.map(f => f.filename + '\x1e' + f.sha256).join('\x1f'),
    encodeField(canonicalConditionalHeaders(input.conditionalHeaders), encode)].join('\x1f'));
}
export const requestDigest = (input: RequestDigestInput, encode: CanonicalEncoder = canonicalize): string => sha256Hex(requestDigestBytes(input, encode));
