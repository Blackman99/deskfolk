...
- **交付前先自查。** Bot 交文件给你之前，先拿规划要点（还没整理出要点时用最初的要求）对一遍已交出的文件和收尾；漏掉的要么补上，要么说清交给谁、为什么不交、什么时候做。
...
- **可分屏的工作台。** 桌面窗口可以分成多块窗格，放会话、终端、日程图、工作区和花费。沿标签栏拖动标签页可调整顺序；右键标签页可关闭它、其他标签页、右侧标签页或所有标签页。空窗格可点右上角 × 关闭，也可在窗格右键菜单选择「关闭窗格」。搜索按钮右边的按钮或 ⌘B 可把会话列表收成只有头像的窄栏。
