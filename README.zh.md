...
- **在交付物上直接批注。** 在预览里点到你说的那一处：一段文字或代码、Markdown 渲染态里的一句、图片或 PDF 页上的一块区域、HTML 页面上的一个元素、音视频的一个时间点。草稿存在本机，攒够一批再发：合成一条引用交付消息的回复并点名交出它的 Bot，Bot 在那个文件所在的工作目录里逐条处理，改完写一句怎么改的把它标成已处理。批注一直挂在文件上，分待处理和已处理；原文变了会标出来。
...
- **不在 Mac 旁，用手机接着管——实验性，默认关闭。** 经你自己部署的中继配对一部手机，连的就是这台 Mac：会话、批准、工作区和终端都在，端到端加密，中间没有 Deskfolk 的云。见[远程访问](#远程访问实验性)。

## 获取

**系统要求：macOS 13.0（Ventura）或更新版本**，支持 Apple 芯片与 Intel。

**下载**最新 Alpha：[GitHub Releases](https://github.com/Blackman99/deskfolk/releases/latest) 提供 Apple 芯片与 Intel 两种未签名 `.dmg`。应用自带运行时与原生 helper，不需要另外装 Bun、Node 或源码。首次打开若被 Gatekeeper 拦截，右键选「打开」，或执行：

```bash
xattr -dr com.apple.quarantine "/Applications/Deskfolk.app"
```

更多说明：[Gatekeeper FAQ](docs/gatekeeper.zh.md) · 公证路径：[docs/notarization.md](docs/notarization.md)（[#10](https://github.com/Blackman99/deskfolk/issues/10)）。
...
手机上，**设置 → 模型服务** 以端点卡片呈现，默认模型使用系统选择器。点卡片的**名单**可搜索和启用模型，点**属性**进入独立页面调整价格、思考等级和擅长领域。改动自动保存，底部显示保存状态，返回逐层退出。
...
手机或另一台电脑上的浏览器，可以经你自己部署的中继连到你 Mac 上的 Deskfolk。Mac 只往外连；中继用 HTTPS 提供手机端页面，并转发它解不开的 Noise 密文。配对过的设备能看会话、回消息、批准或拒绝等你的事、翻工作区并下载 50 MiB 以内的文件、在你的终端里敲命令，还有一条「文件」会话把文件送进工作区 `inbox/`；可选的 Web Push 只发泛化的待办提醒。单人、单台 Mac、最多 16 台设备，Mac 不在时什么都不排队。

接入分三步：部署中继（`deploy/remote/compose.yaml`：Caddy 加 Bun 中继，放在你自己的域名上）；让源码态的 Mac 连上它（`REAL_BOT_DEV_REMOTE=1 pnpm dev`，再用一次性 bootstrap 令牌执行 `bun apps/daemon/scripts/dev-remote.ts init …`）；配对手机（Mac 上 设置 → 通用 → 远控 → **配对新设备**，把一次性配对内容粘到手机上，核对指纹后在 Mac 上批准）。

**默认关闭，也还不是做完的功能：**发布包暂时不能配对，独立安全复核与真机 WebAuthn / Web Push 验收都没有通过。分步指南：[docs/remote-access.zh.md](docs/remote-access.zh.md) · [官网页面](https://blackman99.github.io/deskfolk/zh/remote)。

## 状态

Alpha，仅 macOS。已接入、正在建设与明确不做：[官网](https://blackman99.github.io/deskfolk/zh#boundaries) · [路线图](ROADMAP.md) · [CONTEXT.md](CONTEXT.md)（领域语言）。
