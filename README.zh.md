...
手机上，**设置 → 模型服务** 以端点卡片呈现，默认模型使用系统选择器。点卡片的**名单**可搜索和启用模型，点**属性**进入独立页面调整选路参考价、计费单价、思考等级和擅长领域。改动自动保存，底部显示保存状态，返回逐层退出。
...
账本记录轮次补全、判断、选路、复盘、学习跳和输入建议。删会话、删 Bot 或清空历史都保留花费和名称快照。会话顶栏、列表行和设置沿用现有展示。花费功能不设预算或自动停机，MCP 工具另外收取的费用也无法统计。
