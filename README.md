...
- **Mark up what a bot hands over.** In the preview, annotate the spot you mean: a range of text or code, a phrase in rendered Markdown, a box on an image or a PDF page, an element of an HTML page, or a moment of audio or video. Drafts wait on the Mac until you send the batch, which goes out as one reply that quotes the delivery and names the bot; it works through them in the folder the file came from and marks each one handled with a note. Annotations stay on the file as pending or resolved, and say so when the text they point at has changed.
...
- **Carry on from your phone — experimental, off by default.** Pair a phone through a relay you deploy yourself and it reaches the same Mac: conversations, approvals, the workspace and your terminals, end-to-end encrypted, with no Deskfolk cloud in between. See [Remote access](#remote-access-experimental).
...
A phone, or a browser on another computer, can reach the Deskfolk on your Mac through a relay you run yourself. The Mac only dials out; the relay serves the phone page over HTTPS and forwards Noise-encrypted traffic it cannot read. A paired device reads and answers conversations, approves or denies what is waiting, browses the workspace and downloads files up to 50 MiB, types into your terminals, and has a Files conversation that drops files into the workspace `inbox/`; optional Web Push sends only a generic "pending" reminder. One owner, one Mac, up to 16 devices, nothing queued while the Mac is away.

Setting it up takes three steps: deploy the relay (`deploy/remote/compose.yaml`: Caddy plus the Bun relay, on your own domain), point a source-run Mac at it (`REAL_BOT_DEV_REMOTE=1 pnpm dev`, then `bun apps/daemon/scripts/dev-remote.ts init …` with the one-time bootstrap token), and pair a phone (Settings → General → Remote → **Pair a device** on the Mac, paste the one-time code on the phone, compare fingerprints, approve at the Mac).

**This is off by default and not a finished feature:** a release build cannot pair yet, and the independent security review and real-device WebAuthn / Web Push checks have not passed. Step-by-step guide: [docs/remote-access.md](docs/remote-access.md) · [on the website](https://blackman99.github.io/deskfolk/en/remote).
