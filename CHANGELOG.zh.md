...
- 批注，第三部分（信使：框架与文本）：预览工具栏的「批注」打开这份文件的批注列表，可按全部 / 待处理 / 已处理 / 草稿筛选，点一条定位回原处，草稿能改能删，发出去的能手动标成已处理或重新打开。代码和文本（包括 Markdown、HTML 的源码视图）选中文字后点「批注」，手机上点行号选中一行；行号栏的点和高亮标出每一条。预览底部的发送栏攒着这个对话的草稿，补一句总话一起发出；Bot↔Bot 私聊里的产物会写明发到你和哪个 Bot 的私聊。编辑器里有没保存的修改时不能加批注（从源码切到渲染再切回来也算）；选好位置后文件被改过，存草稿时会拦下并说明。一次最多发 50 条，多了先发最早的 50 条。聊天里那条批次消息下面是一张张批注卡片，点开跳到文件的那一处，Bot 处理完卡片当场变成「已处理」并带上它的说明。预览面板窄的时候，批注列表放到文件下方。
