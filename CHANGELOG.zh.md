...
- 远控未知结果会在断线后仍保留 `{id, method, path, fingerprint}`。重连把同一条内存回执装回新的 RemoteApi，先查 `/v1/requests/:id`；聊天发送遇到 503 `request_unknown` 也不丢这个 id。聊天和 Markdown 的 SVG 缩略图与产物预览共用同一套清理（含 `<svg/onload>`、未闭合 `<script>`、无引号 `javascript:`）。托管/远控设置里工作区路径只读，PATCH 不再带 `workspace_path`。
