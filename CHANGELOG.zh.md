...
- 应用里有终端了，Bot 执行命令的过程也看得见了。侧栏底部一个按钮打开你自己的 shell 会话：cwd 默认在工作区但不关在里面，不走批准——批准管的是 Bot 的危险动作，不是你在自己机器上干的事——Bot 看不见也用不了它。守护进程持有它，所以关窗之后 `pnpm dev` 照跑，退出才停；守护进程没了就标成中断，不假装还活着。另一件事是 Bot 的命令：输出现在就在它那条气泡下面边跑边滚，命令结束折叠成一行——跑了什么、退出码多少、花了多久。Bot 那个壳一点没变：stdin 仍是关的、十分钟超时仍然杀、每次 spawn 仍然重新判定路径，只是不再是一片静默。输出走自己的通道而不是事件流，而且只发给开口要的客户端，所以没人看的构建不用白白搬运。
