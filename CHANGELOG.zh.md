...
- 批注，第二部分（Bot 侧）：带批注的消息会在正文后面把每条批注展开给 Bot——路径、位置、引文或作为图像的裁图、你的意见——原文已变、文件不在了的都标出来。Bot 多了 `list_annotations` 查这件事里还没处理完的批注，`resolve_annotation` 写一句怎么改的把它标掉；读到还有待处理批注的文件会被提醒。轮次指令要求 Bot 逐条处理，不同意就说明原因，不许默默跳过。
