...
- 本机接口 Origin 白名单补上 IPv6 回环 `http://[::1]`。Vite 开发服务器常只绑 `[::1]:5173`，页面 Origin 是 `http://[::1]:5173` 而不是 `http://localhost:5173`；此前健康检查会被 `403 forbidden_origin`，信使只显示「连不上运行时」。`localhost` 与 `127.0.0.1` 不变。
