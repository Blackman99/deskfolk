...
- 打开的是哪个会话现在记在 URL 的 `?s=<id>` 上：刷新、开发态热更新和窗口重载都回到刚才那个会话，浏览器的后退 / 前进在会话之间走。链接指向已经删掉的会话时退回未选状态，不报错。用查询参数而不是 `/s/<id>` 路径，是因为打包后的窗口直接服务静态文件，没有 SPA 回退，路径形式一刷新就 404。设置、抽屉、模型选择记录、工作区和产物预览仍然不进 URL。
