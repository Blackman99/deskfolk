<script lang="ts">
  ...
  import { SCENE_BEATS, stateAt, maxBeat, type MockState, type TranscriptItem } from './scenes';
  ...
  const runningTurns: BotId[] = $derived.by(() => {
    ...
    if (out.length === 0) out.push('writer');
    return out;
  });
</script>
...
<div class="win" bind:this={winEl} style:width="{DESIGN_W}px" style:height="{DESIGN_H}px" aria-hidden="true">
  <div class="frame" class:hidden-to-tray={!!mock.tray}>
    ...
    <aside class="side">
        ...
        {#if mock.groups.length > 0}
          <div class="sect" in:fade={{ duration: dur }}>
            ...
            {#each mock.groups as g (g.id)}
              ...
              >
                <span class="group-glyph">
                  <svg viewBox="0 0 16 16" width="14" height="14"><circle cx="5.5" cy="6" r="2.4" fill="currentColor"/><circle cx="10.8" cy="6" r="2.4" fill="currentColor" opacity=".7"/><path d="M1.5 13c.5-2.4 2.2-3.6 4-3.6s3.5 1.2 4 3.6" fill="currentColor"/><path d="M8.6 13c.4-2 1.6-3.2 3.2-3.2 1.4 0 2.4 1 2.7 3.2" fill="currentColor" opacity=".7"/></svg>
                </span>
      ...
      <div class="side-foot">
        <button class="foot-btn" tabindex="-1">
          <svg viewBox="0 0 16 16" width="13" height="13"><path d="M1.5 4.5A1.5 1.5 0 0 1 3 3h3.2l1.3 1.5H13A1.5 1.5 0 0 1 14.5 6v6A1.5 1.5 0 0 1 13 13.5H3A1.5 1.5 0 0 1 1.5 12z" fill="none" stroke="currentColor" stroke-width="1.4"/></svg>
          <span>{t.mock.workspace}</span>
          <kbd>⌘O</kbd>
        </button>
        <button class="foot-btn icon-only" tabindex="-1" title={t.mock.settings}>
          <svg viewBox="0 0 16 16" width="13" height="13"><circle cx="8" cy="8" r="2.2" fill="none" stroke="currentColor" stroke-width="1.4"/><path d="M8 1.8v1.6M8 12.6v1.6M1.8 8h1.6M12.6 8h1.6M3.6 3.6l1.1 1.1M11.3 11.3l1.1 1.1M3.6 12.4l1.1-1.1M11.3 4.7l1.1-1.1" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
        </button>
      </div>
    </aside>
    ...
    <section class="main">
      <header class="top">
        {#if mock.active === 'coordinator'}
          ...
          <button class="ghost" tabindex="-1">{t.mock.botSettings}</button>
        {:else if mock.active === 'research'}
          <div class="top-id" in:fade={{ duration: dur }}>
            <span class="group-glyph big">
              <svg viewBox="0 0 16 16" width="16" height="16"><circle cx="5.5" cy="6" r="2.4" fill="currentColor"/><circle cx="10.8" cy="6" r="2.4" fill="currentColor" opacity=".7"/><path d="M1.5 13c.5-2.4 2.2-3.6 4-3.6s3.5 1.2 4 3.6" fill="currentColor"/><path d="M8.6 13c.4-2 1.6-3.2 3.2-3.2 1.4 0 2.4 1 2.7 3.2" fill="currentColor" opacity=".7"/></svg>
            </span>
            <div class="top-text">
          ...
          <button class="ghost" tabindex="-1">{t.mock.groupSettings}</button>
        {:else}
          <div class="top-id">
      ...
      <div class="body">
        <div class="transcript" class:narrow={!!mock.preview}>
          {#if mock.active === 'none'}
                        ...
                        <div class="artifacts">
                          {#each item.artifacts as path}
                            <span class="artifact" data-hit={path === t.script.reportPath ? 'artifact-report' : 'artifact-note'}>
                              <svg viewBox="0 0 16 16" width="12" height="12"><path d="M4 1.5h5.5L13 5v9.5H4z" fill="none" stroke="currentColor" stroke-width="1.3"/><path d="M9.5 1.5V5H13" fill="none" stroke="currentColor" stroke-width="1.3"/></svg>
        ...
        {#if mock.preview}
          <div class="preview" transition:fly={{ x: 40, duration: dur }}>
            <div class="pv-head">
              <span class="pv-title">{t.mock.previewTitle}</span>
              <span class="pv-file mono">{t.script.reportPath}</span>
              <span class="pv-toggle"><span>{t.mock.previewRendered}</span><span class="on">{t.mock.previewSource}</span></span>
              <button class="pv-save" class:dirty={mock.preview.edited && !mock.preview.saved} data-hit="preview-save" tabindex="-1">
                {#if mock.preview.edited && !mock.preview.saved}<i class="dot"></i>{/if}
                {t.mock.previewSave}
                <kbd>⌘S</kbd>
              </button>
            </div>
            ...
            {#if mock.preview.saved}
              <div class="pv-toast" in:fly={{ y: 8, duration: dur }}>{t.mock.previewSaved}</div>
            {/if}
          </div>
        {/if}
      </div>
      ...
      <footer class="composer">
        ...
        <div class="composer-hint">{t.mock.sendHint}</div>
      </footer>
    </section>
        ...
        <div class="modal-head">
          ...
          <span class="tabs">
            {#each t.mock.settingsTabs as tab, i}
              <span class="tab" class:on={i === 0}>{tab}</span>
            {/each}
          </span>
        </div>
            ...
            {#if mock.settings >= 5}
              {#each t.script.models as m, i (m.name)}
                <div class="m-row" in:fly={{ y: 6, duration: dur, delay: instant ? 0 : i * 140 }}>
                  <span class="mono m-name">{m.name}</span><span>{m.strengths}</span><span class="mono">{m.thinking}</span><span class="mono">{m.price}</span>
                </div>
              {/each}
            {/if}
          </div>
  ...
  >
    <svg viewBox="0 0 24 24" width="22" height="22"><path d="M5 3l14 8.5-6.2 1.6L9.5 20z" fill="#0f172a" stroke="#fff" stroke-width="1.6" stroke-linejoin="round"/></svg>
    <span class="ripple"></span>
  </div>
</div>

<style>
  /* Window-local tokens: the messenger's own light palette. The mock stays light in both page themes. */
  .win {
    --bg: #eef2f6;
    --pane: #ffffff;
    --sidebar: #f8fafc;
  ...
  .row.active { background: var(--pane); color: var(--ink); font-weight: 600; box-shadow: 0 1px 2px rgba(15, 23, 42, 0.06); }
  ...
  .foot-btn {
    ...
    padding: 5px 8px; border-radius: 7px; border: 1px solid var(--line); background: var(--pane);
    color: var(--ink-2); font-size: 11.5px;
  }
  .foot-btn span { flex: 1; text-align: left; }
  .foot-btn.icon-only { flex: none; width: 28px; justify-content: center; padding: 5px 0; }

  /* ── Main ── */
  .main { display: grid; grid-template-rows: 44px 1fr auto; min-height: 0; background: var(--pane); }
  .top {
  ...
  .top-sub { font-size: 10.5px; color: var(--muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 300px; }
  ...
  .ghost {
    font-size: 11.5px; color: var(--ink-2); padding: 4px 9px;
  ...
  .transcript {
    ...
    padding: 14px 18px 8px; overflow: hidden;
  ...
  .j-chip.join { background: var(--ok-bg); border-color: #a7f3d0; color: var(--ok-text); font-weight: 600; }
  ...
  .ap-btn.primary { background: var(--ink); color: #fff; border-color: var(--ink); }
  .ap-status { font-size: 11px; font-weight: 600; color: var(--ok-text); background: var(--ok-bg); border-radius: 999px; padding: 2px 9px; }

  /* Preview panel */
  .preview {
    width: 380px; flex: none; border-left: 1px solid var(--line); background: var(--pane);
    display: flex; flex-direction: column; min-height: 0; position: relative;
  }
  .pv-head { display: flex; align-items: center; gap: 8px; padding: 8px 10px; border-bottom: 1px solid var(--line); font-size: 11.5px; }
  .pv-title { color: var(--muted); }
  .pv-file { color: var(--ink-2); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .pv-toggle { display: inline-flex; border: 1px solid var(--line); border-radius: 6px; overflow: hidden; font-size: 10.5px; }
  .pv-toggle span { padding: 2px 7px; color: var(--muted); }
  .pv-toggle .on { background: var(--sidebar); color: var(--ink); font-weight: 600; }
  .pv-save { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; padding: 3px 8px; border-radius: 6px; border: 1px solid var(--line); background: var(--pane); color: var(--ink-2); transition: all 200ms ease; }
  .pv-save.dirty { border-color: var(--accent-line); background: var(--accent-tint); color: var(--accent); font-weight: 600; }
  .pv-save .dot { width: 6px; height: 6px; border-radius: 50%; background: var(--accent); display: inline-block; }
  .pv-body { flex: 1; overflow: hidden; padding: 8px 0; font-family: var(--mono); font-size: 11px; line-height: 1.7; }
  .code-line { display: flex; gap: 12px; padding: 0 10px; white-space: pre; }
  .ln { width: 18px; text-align: right; color: var(--muted-2); flex: none; font-variant-numeric: tabular-nums; }
  ...
  .pv-toast {
    ...
    background: var(--ink); color: #fff; font-size: 11px; padding: 6px 10px; border-radius: 8px;
  }

  /* Composer */
  .composer { padding: 6px 18px 10px; border-top: 1px solid var(--line-2); background: var(--pane); }
  .composer-card { border: 1px solid var(--line); border-radius: 12px; background: var(--pane); box-shadow: 0 1px 2px rgba(15, 23, 42, 0.05); padding: 8px 10px 6px; }
  ...
  /* Settings modal */
  .backdrop { position: absolute; inset: 0; background: rgba(15, 23, 42, 0.35); }
  .modal {
    position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);
    width: 540px; background: var(--pane); border-radius: 14px;
    box-shadow: 0 30px 60px -20px rgba(15, 23, 42, 0.45), 0 0 0 1px rgba(15, 23, 42, 0.08);
  ...
  .tab { font-size: 11px; padding: 2px 9px; border-radius: 6px; color: var(--muted); }
  .tab.on { background: var(--pane); color: var(--ink); font-weight: 600; box-shadow: 0 1px 2px rgba(15, 23, 42, 0.08); }
  ...
  .m-row { display: grid; grid-template-columns: 1.1fr 1.3fr 1.2fr 0.7fr; gap: 8px; padding: 5px 10px; font-size: 11px; color: var(--ink-2); border-top: 1px solid var(--line-2); }
  ...
  .btn-dark { background: var(--ink); color: #fff; font-size: 12px; font-weight: 600; padding: 6px 16px; border-radius: 8px; }

  /* New bot sheet */
  .sheet {
    position: absolute; left: 200px; top: 0; bottom: 0; width: 320px;
    background: var(--pane); border-right: 1px solid var(--line);
    box-shadow: 16px 0 36px -6px rgba(15, 23, 42, 0.14);
  ...
  .menubar {
    height: 26px; background: rgba(255, 255, 255, 0.85); border-bottom: 1px solid var(--line);
  ...
  .tray-icon.open { background: rgba(15, 23, 42, 0.12); }
  .tray-menu {
    position: absolute; right: 52px; top: 30px; width: 170px;
    background: rgba(255, 255, 255, 0.96); border: 1px solid var(--line); border-radius: 9px;
    box-shadow: 0 12px 28px -6px rgba(15, 23, 42, 0.25); padding: 4px; font-size: 12px;
  }
  ...
  .ds-turns { display: flex; gap: 14px; flex-wrap: wrap; }
  ...
  .callout {
    ...
    box-shadow: 0 10px 24px -10px rgba(15, 23, 42, 0.35), 0 0 0 1px rgba(15, 23, 42, 0.06);
    transition: left 320ms ease, top 320ms ease;
  }
  .callout::before {
    content: ''; position: absolute; top: 16px; left: -9px;
    border: 6px solid transparent; border-right-color: #146a7c; border-left: 0;
  }
  ...
  .ripple {
    ...
    border: 2px solid #146a7c; opacity: 0; transform: scale(0.4);
  }
