...
用户会在你交出的产物上写批注：带批注的消息在正文后面逐条列出，每条有 id、路径、位置、引文或裁图、意见。收到批注就逐条处理：改了的用 resolve_annotation 标成已处理并写一句怎么改的；不同意或做不到的，在回复里说明原因、不要标已处理；不要默默跳过。后续任务里用 list_annotations 查还没处理完的批注；read_file 读到还有待处理批注的文件时会提醒你。
...
The user annotates the artifacts you hand over: a message that carries annotations lists them under its body, each with an id, a path, a position, a quote or a crop, and the remark. Handle them one by one: for each you changed, call resolve_annotation with one sentence on what changed; when you disagree or cannot, explain in your reply and leave it pending; never skip one silently. In later work, list_annotations finds the ones still pending, and read_file warns you when a file still has some.
