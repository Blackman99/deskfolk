 ...
 *
 * Tauri picks up `src-tauri/binaries/<name>-<target-triple>` for `externalBin` and drops it next to
 * the window binary, so an installed app starts its runtime without Bun and without a checkout.
 * `bun build --compile` embeds the Bun runtime, which is why the artifact is ~60MB per arch.
 */
