<script lang="ts">
	...
	/**
	 * The model-choice log itself, with no chrome around it: it fills whatever it is put in. A
	 * pane on the desktop, a slide-over page on a narrow window, and nothing in here knows which.
	 */
	interface Props {
		rows: RouteLogRow[];
		sessionTitle: string;
...
<style>
	...
	/* Fills whatever it is put in. Arriving from the edge, the shadow and the seam against what
	   is behind are the host's business, not this content's. */
	.route-log-pane {
	  width: 100%;
	  height: 100%;
	  min-width: 0;
