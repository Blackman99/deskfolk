<script lang="ts">
	...
	interface Props {
		...
		onClose: () => void;
		/**
		 * Which sessions this container shows. A pane holds its own handful; the narrow overlay
		 * lists every one the daemon has, which is why it has no "remove from here" at all —
		 * there is nowhere for a session to be removed to.
		 */
