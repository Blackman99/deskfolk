<script lang="ts">
	...
	$effect(() => {
		if (!busy && baseline && live && baseline.updated_at !== live.updated_at && !routineDirty(draft, baseline)) {
	...
	async function mutate(run: () => ReturnType<MessengerRuntime['createRoutine']>, done: () => void): Promise<void> {
		...
		try {
			...
			if (error) failure = routineError(error.status, t);
			else done();
		} catch {
	...
	async function save(): Promise<void> {
		if (!editing || disabled || conflict || missing) return;
		...
		{#if editing}
			<form class="routine-editor" bind:this={editorEl} aria-label={baseline ? t.routines.edit : t.routines.add} onsubmit={(event) => { event.preventDefault(); void save(); }} novalidate>
				...
				<div class="routine-actions">
					...
					<button type="submit" class="btn-primary" disabled={disabled || conflict || missing}>{busy ? t.routines.busy : t.routines.save}</button>
				</div>
			</form>
		{/if}
	</div>
