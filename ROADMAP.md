...
- 一件事记着最初的要求：开这件事的那条要求原文另存，每一轮的局面里都有它、这件事已交出的文件和经过，交接到别的 Bot、别的会话也一样。第一轮标出来，让 Bot 先对齐没说清的地方再动手。判断和输入建议也看得到这件事。
...
- 收尾自检：交付物发给你之前，守护进程打一次无工具短调用，拿这件事最初的要求对照已交出的文件和这条收尾，把既没交出、收尾也没交代去向的要求退回给 Bot 一次；补上或说明去向再发就过。一轮只核一次，不是闸门，退回不算工具错误。
...
- **复用与评估：**选路同时读到这个 Bot 最近干净做完的例子。照做了没有、纠正有没有变少、记下之后同类任务有没有更短，都从已有的行里数出来，写在模型选择记录和 Bot 的记忆、技能行上。要保留依据，而不是把反思文本本身当成进步。目标覆盖评估脚本（`eval:goal-coverage`）拿这件事最初的要求对照交出的文件和 Bot 的收尾，让评判模型逐条判 covered / partial / missing；下一步是把它接进黄金路径验收，按最初的要求逐项验收产物而不是只看有没有文件。
