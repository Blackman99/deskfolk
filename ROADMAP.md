...
- 按 Bot 记录每轮的模型选择与结束方式（完成、端点拒绝、回复不完整、Stop、改道、中断），从用户说到模型本身的跟进消息中收集负反馈，只影响该 Bot 之后同类任务的选择；干净完成的轮次会逐步抵扣惩罚。记录可经本机接口逐会话查看，信使的会话详情里也有一张「模型选择记录」列出每轮选了什么、怎么结束、收到哪些反馈。
